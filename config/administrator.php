...
return array(
     ...
     * 3. 配置信息 -- 必须使用前缀 `settings`,对应的 `settings_config_path` 目录下的文件，如：默认设置下， `settings.site` 访问的是 `administrator/settings/site.php` 文件
     * 4. 页面文件 -- 必须使用前缀 `page`，如：`page.pages.analytics` 对应 `administrator/pages/analytics.php` 或者是 `administrator/pages/analytics.blade.php`，两种后缀名皆可
     * 示例：
     * [
     *     'users',
     ...
     *
     * 此回调函数返回 true 或 false, 用来检测当前用户是否有权限访问后台
     * `true` 为通过， `false` 会将页面重定向至 `login_path` 选项定义的 URL 中
     *
     */
    'permission' => function () {
     ...
     * 当选项 `permission` 权限检测不通过时，会重定向用户至此处设置的路径
     *
     * @type string
     */
